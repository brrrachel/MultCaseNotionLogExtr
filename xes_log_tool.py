...
class XESLogTool:
    ...
    def __create_xes_file__(self, case_notion_columns, extended_tablelog_path ='tableLogs/extended_tableLog.csv') -> EventLog:
        ...
        if (len(case_notion_columns) > 2) or (len(case_notion_columns) == 0):
            print(Fore.RED + 'Error: It is necessary to select one or two columns in order to be able to derive a XES Log file.')
            print(Fore.RED + 'You need to choose them from the following list:', self.extended_tablelog.columns.tolist()[3:])
            exit(0)
    ...
    def get_combined_case_notion(self, case_notion_columns):
        ...
        extracted = self.extended_tablelog[case_notion_columns].values.tolist()
...
if __name__ == '__main__':
    log_extractor = XESLogTool(sys.argv[1:])
